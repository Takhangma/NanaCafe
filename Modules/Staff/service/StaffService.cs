...
public class StaffService
{
    ...
    public async Task<bool> IsRegularCustomer(string email)
    {
        ...
        if (distinctDaysWithOrdersInCurrentMonth >= 2)
        {
            ...
            user.DiscountValidUntil = new DateTime(currentYear, currentMonth == 12 ? 1 : currentMonth + 1, DateTime.DaysInMonth(currentYear, currentMonth == 12 ? 1 : currentMonth + 1));
    ...
    public async Task<bool> IsEligibleForDiscount(string email)
    {
        try
        {
            ...
            Trace.WriteLine("This is Date of Eligible: " + user.DiscountValidUntil);
            if (user.DiscountValidUntil != null)
            {
                if (DateTime.TryParse(user.DiscountValidUntil.ToString(), out discountEligibleUntil))
                {
                ...
                {
                    Trace.WriteLine("Invalid date: " + user.DiscountValidUntil);
                    return false;
                }
            }
            return false;
